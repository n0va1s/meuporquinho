...
select descricao, 'Sugestão de gastos para esta categoria','R', 'MIG',2,id
from tipo
where tipo = 1;

insert into categoria (nom_categoria,txt_categoria,ind_categoria,tip_grupo,seq_usuario,tipo_id)
select descricao, 'Sugestão de gastos para esta categoria','D', 'MIG',2,id
from tipo
where tipo = 0;
...
select 2,1,concat_ws('-',substr(data,7,4),substr(data,4,2),substr(data,1,2)),mes,
