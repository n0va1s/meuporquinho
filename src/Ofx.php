...
class Ofx {
    ...
    // Retorna um array de objetos com as informacoes do banco ou cartao importado
    public function getBank() {
        $xml = new SimpleXMLElement($this->getOfxAsXML());
        $bank = $xml->BANKMSGSRSV1->STMTTRNRS->STMTRS->BANKACCTFROM;
        return $bank;
    }
    // Retorna um array de objetos com as informacoes do cartao de credito
    public function getCard() {
        $xml = new SimpleXMLElement($this->getOfxAsXML());
        $card = $xml->CREDITCARDMSGSRSV1->CCSTMTRS;
        return $card;
    }
}
