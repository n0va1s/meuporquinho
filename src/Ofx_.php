...
$path = "../../../sis_arquivosCliente/banco";
$diretorio = dir($path);
...
function processarArquivo($arquivo) {  
	...
	try{
		...
		$item = array (
				...
				"doc"=>  md5(getValor($value).getData($value)."-".getId($value).getBanco($value)),
				"tipo"=> "0",
			    "banco"=> getBanco($data),
	        ...
	        echo $generico->conexao->insert("Financas_extrato",$item);
...
function getCartao($value){
	if(strchr($value,"CREDITCARDMSGSRSV1")){
		return "1"; // cartão de credito
	}
	else{
		return "0"; // cartão de debito
	}
}
